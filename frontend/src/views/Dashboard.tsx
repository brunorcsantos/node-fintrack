    ...
    <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? 12 : 20 }}>
      ...
      <div style={{
        ...
        <div style={S.statCard("#5AB88A")}>
          ...
          <div style={{ ...S.value, color: "var(--accent-green)", fontSize: isMobile ? 18 : 28 }}>
            {fmt(totalIncome)}
          </div>
        </div>
        <div style={S.statCard("#E85A7A")}>
          <div style={S.label}>Despesas</div>
          <div style={{ ...S.value, color: "var(--accent-red)", fontSize: isMobile ? 18 : 28 }}>
            {fmt(totalExpenses)}
          </div>
        </div>
        <div style={{
          ...
          {pieData.length > 0 ? (
            <>
              ...
              <div style={{ display: "flex", flexWrap: "wrap" as const, gap: 8, marginTop: 12 }}>
                {pieData.map((p) => (
                  <div key={p.name} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "var(--text-secondary)" }}>
                ...
                return (
                  <div key={cat.id}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, gap: 8 }}>
                      <span style={{ fontSize: 13, color: "var(--text-primary)", minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" as const }}>
                        {cat.icon} {cat.name}
                      </span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: cat.color, flexShrink: 0 }}>
                        {fmt(spent)}
                      </span>
                    </div>
                    <ProgressBar value={spent} max={totalExpenses || 1} color={cat.color} />
                  </div>
                );
              })}
            {categories.filter((c) => c.slug !== "receita" && expenseByCategory[c.id]).length === 0 && (
      ...
      <div style={isMobile ? S.cardMobile : S.card}>
        ...
        {filteredTx.length === 0 ? (
          <div style={{ color: "var(--text-muted)", textAlign: "center", padding: "24px 0", fontSize: 14 }}>
              ...
              <div key={tx.id} style={S.txRow}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>
                    {fmtDate(tx.date)} · {cat?.name}{sub ? ` › ${sub.name}` : ""}
                  </div>
                </div>
                <span style={{
                  fontWeight: 700,
